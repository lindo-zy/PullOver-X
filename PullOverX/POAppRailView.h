...
//  拖拽关闭小窗,长按手势转发用于调出原快速切换菜单。贴边竖栏最底部固定
//  一个把手左右切换按钮(受"把手左右切换"开关控制),点击后把手换边、
//  竖直位置不变。
//  开启"把手展开居中"后,小窗展开时第一个图标钉在竖栏顶部、其余图标下移到
//  屏幕中部,此时控制器会把竖栏 frame 撑满上下可用空间;缩点把手唤出的
//  关闭态竖栏由控制器决定排布:开启"缩点竖栏悬浮居中"时以放大一倍的图标
//  网格悬浮在屏幕中央,网格在屏幕上固定从左往右正向排列(左手模式的整窗
//  镜像由视图内部反向抵消),底部铺一层
//  毛玻璃底衬与当前界面区分,不显示换边按钮;超出一行自动折成多排,超高时
//  竖向滚动,键盘弹出时上移到键盘上方。否则贴把手边缘列、竖向紧凑排布,
//  换边按钮照常钉在竖栏底部。排布形态均由控制器通过 centeredIcons/
//  horizontalLayout/frame 写入。
//
...
// 横排网格在宽度上限内折行后的内容尺寸(悬浮网格不显示换边按钮,不含其
// 占位),供控制器确定悬浮网格的 frame;列数与 layoutTiles 的折行算法一致。
