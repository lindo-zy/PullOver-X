...
Tapping the nubbed (shrunk) handle reveals the app rail floating at the center of the screen as a grid of doubled-size icons; icons wrap into multiple rows and scroll when they do not fit. While the keyboard is visible, the rail automatically moves above the keyboard and falls back to the center when it hides. Toggle it in settings ("Float Nub Rail Centered"); turning it off keeps the rail in the handle's edge column.
